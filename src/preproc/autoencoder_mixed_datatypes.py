...
def run_autoencoder(df, cols_cont, cols_binary, output_path, epochs=50, batch_size=32, latent_dim=6, random_state=42):
    if df.isnull().values.any():
        raise ValueError('Input DataFrame contains NaN values.')
    
    cont_indices = [df.columns.get_loc(col) for col in cols_cont]
    binary_indices = [df.columns.get_loc(col) for col in cols_binary]
    ...
    autoencoder.compile(optimizer=Adam(learning_rate=0.001, clipnorm=1.0), loss=custom_loss)

...
def preproc_pipeline(df):
    ...
    df_disc = df[customer_info_cols_disc]
    df_binary = df[customer_info_cols_binary]
    ...
    df_cont_scaled = pd.DataFrame(cont_scaler.transform(df_cont_sqrt), columns=df_cont_sqrt.columns, index=df_cont_sqrt.index)
    
    disc_imputer = SimpleImputer(strategy='most_frequent')
    disc_imputer.fit(df_disc)
    df_disc_imputed = pd.DataFrame(disc_imputer.transform(df_disc), columns=df_disc.columns, index=df_disc.index)
    
    disc_binned = binning(df_disc_imputed)
    
    df_preproc = pd.concat([df_cont_scaled, disc_binned, df_binary], axis=1)

...
def preproc_pipeline_customer_info(df, scaler=MinMaxScaler()):
    ...
    #df_filtered, b = remove_outliers_iqr(df_filtered, df_filtered.columns)
    ...
    run_autoencoder(df_scaled, 'data/processed/latent_representation_withoutliers.csv', epochs=50, batch_size=32, latent_dim=4)
...
customer_info_preproc = preproc_pipeline_customer_info(customer_info_clean, scaler=MinMaxScaler())

base_dir = 'data/processed'
customer_info_preproc.to_csv(os.path.join(base_dir, 'customer_info_preproc_withoutliers.csv'))
